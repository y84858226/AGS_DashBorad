...
function createPLChart(result,type){
	...
	if(type=="week"){
		...
		for (k in array) {
			$.each(result,function(index,value){
				var gl=value.productGroupDescription;
				var week=value.week;
				var coverage=value.coverage;
				if(array[k]==gl){
					data[number]=new Array();
		...
		success : function(data) {
			...
			$.each(array,function(n1,v1){
				...
				$.each(data,function(n2,v2){
					if(v1==v2.pl){
						html+='<option value="'+v2.productGroupDescription+'">'+v2.productGroupDescription+'</option>';
					}
				})
				html+='</optgroup>';
			})
...
$(document).ready(function () {
	...
	$("#DownLoadCoverage").on('click',function(){
		...
		var downLoadURL="./downLoadBrowseNodeCoverage.spring?sourceCountry="
	...
	//下载DefectiveAsin
	$("#DownLoadDefectiveAsin").on('click',function(){
		var sourceCountry=$("#SourceCountry").val();
		var targetCountry=$("#TargetCountry").val();
		if(targetCountry=="CN"){
			var year=$("#Year").val();
			...
			var pls=$("#PL").selectpicker('val');
			var gls=$("#GL").selectpicker('val');
			var downLoadURL="./downLoadBrowseNodeDefectiveAsin.spring?sourceCountry="
				  +sourceCountry+"&targetCountry="+targetCountry+"&year="+year+"&weeks="+weeks;
			if(pls!=null){
				downLoadURL+="&pls="+pls;
			}
			...
			alert("The function is only supported Chinese Market");
		}
	})
